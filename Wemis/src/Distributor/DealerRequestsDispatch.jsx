...
import { X, Send, Package, IndianRupee, Loader2, User, ChevronRight, CheckCircle2, MapPin, Check, Info, ShieldCheck } from 'lucide-react';
...
function DealerRequestsDispatch() {
    ...
    const DISPATCH_API = "https://api.websave.in/api/manufactur/sendWalletDistributorToDeler";
        ...
        try {
            ...
            if (response.data.success) {
                ...
                setFormData({
                    requestId: item._id,
                    state: apiData.state || '',
                    dealerName: apiData.partnerName || item.dealerName,
                    qty: apiData.requestedWalletCount || item.requestedWalletCount,
                    price: item.totalPrice || 0,
                    planId: apiData.activationPlanId || '',
                    distDelerId: item.DistributordelerId // Captured from GET_HISTORY
                });
                setIsModalOpen(true);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                distDelerId: formData.distDelerId,
            ...
            if (response.data.success) {
                ...
                fetchHistory(); // Sync background data
            } else {
                toast.error(response.data.message || "Dispatch Failed", { id: tid });
            }
        } catch (error) {
            toast.error("An error occurred during dispatch", { id: tid });
        } finally {
            setSubmitting(false);
        }
    };
        ...
        <div className="min-h-screen bg-[#F8FAFC] pb-12 font-sans text-slate-900">
            ...
            <div className="max-w-full mx-auto p-6 mt-4">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-slate-900">Dealer Requests</h1>
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {loading ? (
                                    ...
                                    // Check if status is "completed" from API or local state
                ...
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
                        <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center">
                            <div>
                                <h3 className="text-xl font-bold text-slate-900 uppercase tracking-tight">Stock Dispatch</h3>
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
                                    Dealer: <span className="text-indigo-600">{formData.dealerName}</span>
                                </p>
                            </div>
                                ...
                                <div className="space-y-6">
                                    <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-start gap-3">
                                        ...
                                        <p className="text-[11px] text-indigo-700 leading-relaxed font-bold uppercase tracking-tight">
                                            Verify quantities carefully. Dispatching stock is an irreversible action in the ledger.
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-5">
                                        <div>
                                            ...
                                            <div className="relative group">
                                                ...
                                                <input required name="state" placeholder="e.g. Maharashtra" value={formData.state} onChange={handleInputChange} className="w-full bg-white border border-slate-200 p-3.5 pl-10 rounded-xl font-bold text-slate-700 focus:border-indigo-500 outline-none transition-all" />
                                            </div>
                                        </div>

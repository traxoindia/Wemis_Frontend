...
// --- Utility Components for a Clean Look ---
...
const NavDropdown = ({ menuKey, title, icon, children, openMenu, toggleMenu }) => (
  <div className="relative">
    <button
      onClick={() => toggleMenu(menuKey)}
      className={`flex items-center gap-1 px-4 py-2 rounded-lg transition-all duration-200 ${ // Padding adjusted for larger button
        openMenu === menuKey 
    ...
    >
      ...
      <FaChevronDown className={`w-2 h-2 ml-1 transform transition-transform duration-200 ${
        openMenu === menuKey ? "rotate-180" : "rotate-0"
      }`} />
    </button>
    {openMenu === menuKey && (
...
// Elegant Stat Card Component (Font and icon sizes small)
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      {/* --- Navbar (Big and Elegant) ---
        Changed h-14 to h-20 (or custom padding) for a bigger look.
        Used py-4 on the flex container instead of h-class for better control.
      */}
      <div
        ref={navRef}
        className="sticky top-0 z-30 bg-gray-900 border-b border-yellow-500/30 shadow-2xl shadow-gray-900/50"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4"> {/* Added vertical padding (py-4) for a bigger bar */}
          <div className="flex justify-between items-center">
            {/* LOGO (Size adjusted) */}
            <Link to="/dealer/dashboard" className="text-3xl font-extrabold tracking-wider text-yellow-500">
              WeMIS
            </Link>
            ...
            <nav className="hidden md:flex items-center gap-2 text-xs">
              <Link
                to="/dealer/dashboard"
                className="flex items-center gap-1 px-4 py-2 rounded-lg text-yellow-500 bg-gray-800 transition-colors duration-200 font-semibold shadow-md" // Padding adjusted
              >
                <FaTachometerAlt className="text-sm" /> Dashboard
              </Link>

              <NavDropdown menuKey="barcode" title="Barcode" icon={<FaBarcode />} openMenu={openMenu} toggleMenu={toggleMenu}>
                <DropdownLink to="/dealer/scan-barcode">Scan Barcode</DropdownLink>
                <DropdownLink to="/dealer/barcode-list">Barcode List</DropdownLink>
              </NavDropdown>

              <NavDropdown menuKey="members" title="Members" icon={<FaUsers />} openMenu={openMenu} toggleMenu={toggleMenu}>
                <DropdownLink to="/dealer/technicians">Technicians</DropdownLink>
                <DropdownLink to="/dealer/dealers">Dealers</DropdownLink>
              </NavDropdown>

              <NavDropdown menuKey="device" title="Manage Device" icon={<FaCogs />} openMenu={openMenu} toggleMenu={toggleMenu}>
                <DropdownLink to="/dealer/map-device">Map Device</DropdownLink>
                <DropdownLink to="/dealer/device-stock">Device Stock</DropdownLink>
              </NavDropdown>
            </nav>
            ...
            <div className="flex items-center gap-3">
              {/* Product */}
              <button className="hidden sm:block bg-yellow-500 text-gray-900 px-3 py-1 rounded-full font-bold text-xs shadow-lg hover:bg-yellow-400 transition-colors duration-150">
                Product
              </button>
              
              {/* Wallet Dropdown (Icon Only) */}
              <NavDropdown menuKey="wallet" title="" icon={<FaWallet />} openMenu={openMenu} toggleMenu={toggleMenu}>
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-2 bg-gray-800 p-5 rounded-xl shadow-2xl">
                ...
                <div className="space-y-3">
                    {[
                        { action: "Device #XYS900 Mapped", detail: "Technician A", time: "2 min ago", color: "text-green-400" },
                        { action: "Wallet Top-up Success", detail: "₹1,000 added", time: "1 hour ago", color: "text-yellow-500" },
                        { action: "Renewal Due Reminder", detail: "Device #A34B12", time: "3 hours ago", color: "text-red-400" },
                        { action: "New Dealer Registered", detail: "Dealer ID 789", time: "Yesterday", color: "text-blue-400" },
                        { action: "Device #QRZ345 Mapped", detail: "Technician B", time: "Yesterday", color: "text-green-400" },
                    ].map((item, i) => (
                        <div key={i} className="flex justify-between items-center py-1.5 border-b border-gray-700 hover:bg-gray-700/50 px-2 -mx-2 rounded-md transition-colors">
                            <span className={`text-sm font-medium ${item.color}`}>{item.action}</span>
                            <div className="text-right">
                                <span className="block text-xs text-gray-300">{item.detail}</span>
            ...
            <div className="bg-gray-800 p-5 rounded-xl shadow-2xl flex flex-col justify-between">
                <div>
                    ...
                    <div className="space-y-3">
                        <Link to="/dealer/renewal-list" className="w-full inline-flex items-center justify-center px-3 py-2 border border-yellow-500 text-xs font-bold rounded-lg text-yellow-500 bg-gray-900 hover:bg-yellow-500 hover:text-gray-900 transition-colors shadow-lg">
                            <FaClock className="mr-2"/> View Renewal Status
                        </Link>
                        <Link to="/dealer/add-technician" className="w-full inline-flex items-center justify-center px-3 py-2 border border-transparent text-xs font-bold rounded-lg shadow-sm text-white bg-green-600 hover:bg-green-700 transition-colors">
                            <FaUsers className="mr-2"/> Add New Technician
                        </Link>
                        <Link to="/dealer/wallet" className="w-full inline-flex items-center justify-center px-3 py-2 border border-transparent text-xs font-bold rounded-lg shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 transition-colors">
      ...
      <footer className="mt-8 py-3 border-t border-gray-800 text-center text-xs text-gray-600">
        &copy; {new Date().getFullYear()} WeMIS. All rights reserved. | Dealer Panel v1.2
      </footer>
    </div>
  );
};
...
// Add this CSS to your global stylesheet (e.g., index.css or global.css)

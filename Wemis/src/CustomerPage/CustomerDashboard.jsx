...
import { 
  ...
  StopCircle, Activity, Play, Pause, History, Calendar, CheckCircle2, AlertTriangle, MoreHorizontal
} from "lucide-react";
...
const CustomerDashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on("gps-update", (data) => {
      setDevices(prev => {
        ...
        if (selectedDevice && selectedDevice.deviceNo === data.deviceNo && !isPlaybackMode) {
            setSelectedDevice(prevDev => ({ ...prevDev, ...data, lat: Number(data.lat), lng: Number(data.lng), speed: Number(data.speed || 0), heading: Number(data.heading || 0) }));

            if (leafletMapRef.current && vehicleMarkerRef.current) {
                const newLatLng = [Number(data.lat), Number(data.lng)];
                const newHeading = Number(data.heading || 0);
                
                vehicleMarkerRef.current.setLatLng(newLatLng);
                const iconHtml = createVehicleIcon(newHeading);
                const newIcon = window.L.divIcon({ className: 'vehicle-marker', html: iconHtml, iconSize: [40, 40], iconAnchor: [20, 20] });
  ...
  const fetchRoutePlayback = async () => {
    if (!selectedDevice?.deviceNo || !playbackStartTime || !playbackEndTime) return alert("Please select date range");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-150px)] min-h-[600px]">
            ...
            <div className="lg:col-span-8 bg-white rounded-[2.5rem] p-3 shadow-xl border border-slate-200 relative overflow-hidden flex flex-col group">
                ...
                {isPlaybackMode && playbackRoute.length > 0 && (
                    <div className="absolute bottom-6 left-6 right-6 z-[1001] bg-white/90 backdrop-blur-md p-4 rounded-3xl shadow-xl border border-white/50">
                        <input 
                            type="range" min="0" max={playbackRoute.length - 1} value={playbackIndex}
            ...
            <div className="lg:col-span-4 flex flex-col gap-4 overflow-y-auto pr-1">
                <div className="grid grid-cols-2 gap-3">
                    <StatCard icon={<Gauge size={18} />} label="Speed" val={displayData?.speed || 0} unit="km/h" color="text-amber-500" bg="bg-amber-50" border="bg-amber-500"/>
                    <StatCard icon={<Compass size={18} />} label="Heading" val={displayData?.heading || 0} unit="°" color="text-blue-500" bg="bg-blue-50" border="bg-blue-500"/>
                    <StatCard icon={<BatteryCharging size={18} />} label="Battery" val={selectedDevice.externalBattery || 12.4} unit="V" color="text-green-500" bg="bg-green-50" border="bg-green-500"/>
                    <StatCard icon={<Signal size={18} />} label="Signal" val={selectedDevice.gsmSignal || selectedDevice.rssi || 0} unit="%" color="text-purple-500" bg="bg-purple-50" border="bg-purple-500"/>
                </div>
                ...
                <div className="flex-1 bg-white rounded-[2rem] p-5 border border-slate-200 shadow-sm flex flex-col">
                    <div className="flex items-center gap-3 mb-5">
                        <div className="p-2 bg-rose-50 rounded-xl text-rose-600"><BarChart3 className="w-5 h-5" /></div>
                        <div>
                                                ...
                                                {data.map((item, idx) => (
                                                    <div key={idx} className="bg-white p-2.5 rounded-lg border border-slate-100 text-[10px] shadow-sm flex flex-col gap-1">
                                                        <div className="flex justify-between font-bold text-slate-700">
                                                            <span className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-600">{item.time || item.start}</span>
                                                            <span className="text-indigo-600">{item.duration || item.status || item.dist}</span>
                                                        </div>
                                                        {item.location && <div className="text-slate-400 truncate flex items-center gap-1"><MapPin size={8}/> {item.location}</div>}
                                                    </div>
                                                ))}
                                                {data.length === 0 && <div className="text-[10px] text-slate-400 text-center italic py-2">No recorded events today.</div>}
                                            </div>
                                        </div>
                                    )}
                                </div>
